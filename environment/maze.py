...
class Maze:
    ...
    radius_reward = 2.5
    def __init__(self, maze,rew='Base',rad=False, num_exit=1, start_cell=(0, 0), exit_cell=None):
        ...
        end_list = self.empty.copy()
        self.__exit_cell = []
        self.__visited_exit_cell= []
        self.__radius_cells=[]

        self.radius_flag=rad
        for i in range(len(exit_cell)):
            #e=random.choice(end_list)
            e=tuple(exit_cell[i])
            #print(e)
            self.__exit_cell.append(e)
            #(self.empty)
            self.empty.remove(e)
            #print(self.empty)

            if tuple(((e[0],e[1]+1))) in self.empty:
                self.__radius_cells.append(tuple((e[0],e[1]+1)))
            
            if tuple(((e[0]+1,e[1]))) in self.empty:
                self.__radius_cells.append(tuple((e[0]+1,e[1])))
            ...
            #print(self.__radius_cells)
            #exit()
            #self.maze[e[0],e[1]]=Cell.END        
            #if self.maze[start_cell[0]][start_cell[1]+1] != Cell.OCCUPIED and (start_cell[0],start_cell[1]+1) in self.cells :

        # Check for impossible maze layout
        if all(elem in self.__exit_cell for elem in self.cells):
            raise Exception("Error: exit cell at {} is not inside maze".format(self.__exit_cell))
        if any(elem == Cell.OCCUPIED for elem in self.__exit_cell):
    ...
    def reset(self, start_cell=(0, 0)):
        ...
        if self.__render in (Render.TRAINING, Render.MOVES):
            ...
            for i in range(len(self.__exit_cell)):
                #print(self.__exit_cell)
                self.__ax1.plot(self.__exit_cell[i][0],self.__exit_cell[i][1], "gs", markersize=30)  # exit is a big green square
                self.__ax1.text(self.__exit_cell[i][0],self.__exit_cell[i][1], s="Exit", ha="center", va="center", color="white")
    ...
    def __execute(self, action):
        ...
        if not possible_actions:
            ...
            if (self.__current_cell in self.__exit_cell) and (self.__current_cell not in self.__visited):
            ...
            elif self.__current_cell in self.__radius_cells and self.radius_flag is True:
                #print("not")
                reward = Maze.radius_reward
            else:
                if self.rew=="Base":
                    reward = Maze.penalty_move #-((distance.euclidean(self.__current_cell, self.__exit_cell[0]))/(len(self.maze[0])*len(self.maze[0]*len(self.maze[0]))))  # penalty for a move which did not result in finding the exit cell
                elif self.rew=="Euc":
                    reward =  -(distance.euclidean(self.__current_cell, self.__exit_cell[-1])/(len(self.maze[0])*20))
                elif self.rew=="Man":
                    reward =  -(distance.cityblock(self.__current_cell, self.__exit_cell[-1])/(len(self.maze[0])*20))
                elif self.rew=="Che":
                    reward =  -(distance.chebyshev(self.__current_cell, self.__exit_cell[-1])/(len(self.maze[0])*20))
                #`print(reward)
    ...
    def __status(self):
        ...
        if all(elem in self.__visited for elem in self.__exit_cell):
            return Status.WIN
    ...
    def play(self, model, start_cell=(0, 0)):
        ...
        while True:
            ...
            cnt+=1
            lst.append(tuple((action,state)))
            if status in (Status.WIN, Status.LOSE):
                return status,cnt,lst
    ...
    def check_win_all(self, model):
        ...
        for cell in self.empty:
            status,_,_=self.play(model, cell)
    ...
    def render_q(self, model):
        ...
        if self.__render == Render.TRAINING:
            ...
            for i in range(len(self.__exit_cell)):
                self.__ax2.plot(self.__exit_cell[i][0],self.__exit_cell[i][1], "gs", markersize=30)  # exit is a big green square
                self.__ax2.text(self.__exit_cell[i][0],self.__exit_cell[i][1], s="Exit", ha="center", va="center", color="white")
